...
Сайт решает задачу «отвязать бизнес от личных связей Марии»: сделать SEO-оптимизированные страницы услуг и кейсов, чтобы клиенты, ищущие «организация корпоратива Калининград» и аналогичные запросы, попадали на сайт и оставляли заявки. Сайт собран на Astro (Cloudflare Pages): тексты, структура и SEO живут здесь, в репозитории, и публикуются на крылья.life автоматически через `git push`.
...
**Стек:** Astro 6 + Tailwind v4 + MDX + Content Collections, деплой **Cloudflare Pages** (автодеплой из GitHub, ветка `main`). Все правки — в Markdown/MDX в этой папке → `git push` → автодеплой. Tilda отключена, кабинет больше не трогаем.

**Текущее состояние:** крылья.life **живёт на новом Astro-сайте** (переезд с Tilda завершён в Phase 7), **проиндексирован** в Яндексе и Google, подключены Я.Вебмастер, Google Search Console и Метрика. `git push` в `main` публикует изменения на крылья.life автоматически (обычно 1–2 минуты). Актуальный статус проекта — всегда в [.planning/STATE.md](.planning/STATE.md).
